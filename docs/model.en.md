...
2. **Candidates**:
   ...
   - **Chain compaction**: a cut plane is taken along X or Z (at midpoints between consecutive equipment coordinates on that axis) and every movable device on one side of it is translated **as a whole** towards the other side. The travel is the smallest compressible slack over all pipes crossing the cut (their two ports' separation along that axis minus the minimum straight length), tried at 1, 1/2 and 1/4 of it, and capped by the equipment itself: the group must stop before its boxes come within the clearance of any device or pipe outside it. Every cut is scored (travel x number of pipes that get shorter) and only the few best are actually evaluated each round -- a cut that has been squeezed has less slack left, so the next round naturally moves on to somewhere else. pipes with both ends on the same side ride along unchanged. The first two kinds only ever move perpendicular to a pipe's run, so they remove elbows but never shorten a straight section; this one exists to shrink the overall footprint. The travel is only an estimate — whether the compacted layout still clears the equipment and has room to route is decided by the candidate's re-route and the validator.
   - **Compaction takes whole rounds per axis**: within one round the two axes crowd each other out — the axis with more pipes always scores higher, and the two device groups overlap too much to be accepted in the same batch, so the smaller axis comes second every round and its opportunity is gone by the time the larger one is done. **Which axis goes first** also matters a lot (12% of footprint area in our measurements) and the two usually score close enough that it cannot be decided in advance, so each ordering is run once and the better result (validator's cost) is kept, with `seconds` split between the passes.
